...
// Loader — stays visible until Supabase data is fully fetched
function Loader() {
  return (
    <motion.div
...
export default function PortfolioPage() {
  ...
  // Wait for the real Supabase data — no more placeholder flash
  const { loading: dataLoading } = usePortfolio();

  // Minimum 600ms so the loader never flickers in/out on fast connections
      ...
      {!loading && (
        <>
          ...
          <motion.button
            ...
            className="fixed bottom-8 right-8 w-10 h-10 bg-bg-card border border-border-bright rounded-full flex items-center justify-center text-text-muted hover:text-accent-gold hover:border-accent-gold/50 transition-all z-40 shadow-card text-lg"
