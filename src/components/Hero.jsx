...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center overflow-hidden dot-grid">
      ...
      {/* Background Decor */}
      <div className="glow-blob w-[300px] h-[300px] md:w-[600px] md:h-[600px] opacity-30 md:opacity-40" 
           style={{ background: 'var(--color-accent)', top: '-100px', right: '-100px' }} />
      <div className="glow-blob w-[250px] h-[250px] md:w-[400px] md:h-[400px] opacity-15 md:opacity-20" 
      ...
      <div className="section-container relative z-10 pt-28 pb-20 w-full">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          {/* Right visual (Moved up in DOM order for mobile stack, kept right on desktop) */}
          <motion.div
                ...
                {info.avatar_url ? (
                  ...
                  <>
                    <div className="absolute inset-0 flex flex-col justify-center items-start gap-2 p-6 md:p-8 opacity-20">
                      {[`const me = {`, `  name: "${info.first_name}",`, `  passion: "code",`, `};`].map((l, i) => (
                        <p key={i} className="font-mono text-[10px] md:text-xs" style={{ color: 'var(--color-accent)' }}>{l}</p>
                      ))}
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="font-display font-extrabold text-5xl md:text-6xl opacity-20" style={{ color: 'var(--color-accent)' }}>
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="flex flex-col gap-6">
            ...
            <motion.div variants={item} className="font-display text-xl md:text-2xl font-semibold text-text-secondary h-8 text-center lg:text-left">
              <TypingText strings={typingStrings} />
            </motion.div>

            <motion.p variants={item} className="font-body text-text-secondary text-sm md:text-lg leading-relaxed max-w-lg mx-auto lg:mx-0 text-center lg:text-left">
              {info.bio}
            </motion.p>

            {/* CTAs */}
            <motion.div variants={item} className="flex flex-wrap items-center justify-center lg:justify-start gap-3 mt-2">
              <motion.button
            ...
            <motion.div variants={item} className="flex flex-col gap-2 mt-4 items-center lg:items-start">
              {allSocials.map(({ platform, label, href }) => (
                <motion.a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="font-mono text-[10px] md:text-xs text-text-muted hover:text-text-primary transition-colors duration-300 flex items-center gap-2"
                  whileHover={{ x: 5 }}>
              ...
              {info.email && (
                <div className="flex items-center gap-2 mt-2 opacity-60">
                   <span className="hidden lg:block w-8 h-px" style={{ background: 'var(--color-border-bright)' }} />
                   <span className="font-mono text-[10px] md:text-xs text-text-muted">{info.email}</span>
                </div>
              )}
            </motion.div>
      ...
      >
        <span className="font-mono text-[10px] tracking-widest">{config.hero_scroll_label}</span>
